...
def get_introns_no(f, chromosome, start, end):
    samfile = pysam.AlignmentFile(f, "rb") 
...
def shortest_distance(short, iso, ref, chromosome, counts):
    equal = iso.intersection(short)
    short = short.difference(equal)
    iso = iso.difference(equal)
    dist = 1000000000000
...
for gene in gene_list:
    if overlaps((gene.start,gene.end), current_region) and gene.seqid == chromosome: # Chromosom mit abfragen?
        ...
        if genes : 
            ...
            #paired_c.extend(p)
            #onlyshort += s
            shortest_distance(short_int, iso_int, ref_int, chromosome, counts)
        genes = [gene]
